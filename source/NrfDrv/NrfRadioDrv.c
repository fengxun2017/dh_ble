...
#define PCNF1_MAXLEN_VALUE			(BLE_PDU_PAYLOAD-2)	//51822的radio PDU中的前两字节不算在payload长度内
...
void NrfRadioInit(void)
{
	...
	NRF_RADIO->CRCCPOLY = CRCPOLY_BLE_POLY;
...
void NrfLogicAddrCfg(u1 u1LogicAddr, u4 u4PhyAddr)
{
	...
	if ( LOGIC_ADDR_0 == u1LogicAddr )
	{
		...
		NRF_RADIO->PREFIX0 &= ~(0xFF<<PREFIX0_AP0_POS);
	...
	else if( u1LogicAddr <= LOGIC_ADDR_7 )
	{
		...
		{
			case LOGIC_ADDR_1:
				NRF_RADIO->PREFIX0 &= ~(0xFF<<PREFIX0_AP1_POS);
				NRF_RADIO->PREFIX0 |= (u1Prefix<<PREFIX0_AP1_POS);
			break;

			case LOGIC_ADDR_2:
				NRF_RADIO->PREFIX0 &= ~(0xFF<<PREFIX0_AP2_POS);
				NRF_RADIO->PREFIX0 |= (u1Prefix<<PREFIX0_AP2_POS);
			break;
			
			case LOGIC_ADDR_3:
				NRF_RADIO->PREFIX0 &= ~(0xFF<<PREFIX0_AP3_POS);
				NRF_RADIO->PREFIX0 |= (u1Prefix<<PREFIX0_AP3_POS);
			break;
			
			case LOGIC_ADDR_4:
				NRF_RADIO->PREFIX1 &= ~(0xFF<<PREFIX1_AP4_POS);
				NRF_RADIO->PREFIX1 |= (u1Prefix<<PREFIX1_AP4_POS);
			break;
			
			case LOGIC_ADDR_5:
				NRF_RADIO->PREFIX1 &= ~(0xFF<<PREFIX1_AP5_POS);
				NRF_RADIO->PREFIX1 |= (u1Prefix<<PREFIX1_AP5_POS);
			break;
			
			case LOGIC_ADDR_6:
				NRF_RADIO->PREFIX1 &= ~(0xFF<<PREFIX1_AP6_POS);
				NRF_RADIO->PREFIX1 |= (u1Prefix<<PREFIX1_AP6_POS);
			break;
			
			case LOGIC_ADDR_7:
				NRF_RADIO->PREFIX1 &= ~(0xFF<<PREFIX1_AP7_POS);
